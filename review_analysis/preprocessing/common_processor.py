...
def _texts_to_corpus(
    texts, extra_stopwords: Optional[Set[str]] = None
) -> List[str]:
    """원본 텍스트 시퀀스를 TF-IDF 입력용 토큰 문자열 리스트로 변환한다.

    CSV 경로와 MongoDB 경로가 동일한 정제·토큰화 기준을 공유하도록
    이 함수 하나로 통일한다. 기준이 갈리면 같은 리뷰가 두 경로에서
    다른 vocabulary를 만들어 결과 비교가 불가능해진다.

    Args:
        texts: 원본 리뷰 본문의 시퀀스 (Series, list, generator 모두 가능).
        extra_stopwords: 기본 불용어에 추가로 제거할 단어 집합.

    Returns:
        공백으로 구분된 토큰 문자열 리스트. 토큰이 0개인 항목은 제외된다.
    """
    corpus: List[str] = []
    for raw in texts:
        if raw is None:
...
class _CorpusVectorizer:
    """리뷰 코퍼스를 받아 TF-IDF + SVD를 fit하는 벡터라이저.

    사이트별로 각자 fit하면 vocabulary와 컬럼 인덱스가 달라져
    "A의 3번 컬럼"과 "B의 3번 컬럼"이 서로 다른 단어를 가리키게 된다.
    따라서 세 사이트를 합친 공통 코퍼스로 한 번만 fit한다.

    코퍼스를 **어디서 수집하는지는 이 클래스가 알지 않는다.** CSV(로컬 CLI)든
    MongoDB(API)든 호출 측이 수집해서 넘긴다. 저장소가 바뀌어도 벡터화
    로직은 그대로 재사용된다.
    """
    ...
    def __init__(self, corpus: List[str]):
        if not corpus:
            raise ValueError("코퍼스가 비어 있어 TF-IDF를 fit할 수 없습니다.")

        self.vectorizer = TfidfVectorizer(
            max_features=TFIDF_MAX_FEATURES,
            min_df=min(TFIDF_MIN_DF, len(corpus)),
            token_pattern=r"\S+",
        )
    ...
    def build_corpus_from_csv(
        database_dir: str, extra_stopwords: Optional[Set[str]] = None
    ) -> List[str]:
        corpus: List[str] = []
        ...
        for path in sorted(glob.glob(pattern)):
            ...
            if "content" not in df.columns:
                continue
            corpus.extend(_texts_to_corpus(df["content"], extra_stopwords))
        return corpus

    @classmethod
    def get_or_create(cls, key: str, corpus_factory) -> "_CorpusVectorizer":
        """키 단위로 캐시된 인스턴스를 반환한다.

        corpus_factory는 캐시 미스일 때만 호출되므로, 코퍼스 수집 비용
        (수천 건 형태소 분석)이 중복 발생하지 않는다.
        """
        if key not in cls._cache:
            cls._cache[key] = cls(corpus_factory())
...
class ReviewProcessor(BaseDataProcessor):
    ...
    def _get_vectorizer(self) -> "_CorpusVectorizer":
        ...
        기본 구현은 database 폴더의 CSV들로 코퍼스를 구성한다.
        MongoDB 등 다른 저장소를 쓰는 하위 클래스는 이 메서드를
        오버라이드해 코퍼스 소스만 교체한다.
        """
        database_dir = os.path.dirname(os.path.abspath(self.input_path))
        return _CorpusVectorizer.get_or_create(
            key=f"csv:{os.path.abspath(database_dir)}",
            corpus_factory=lambda: _CorpusVectorizer.build_corpus_from_csv(
                database_dir, self.EXTRA_STOPWORDS
            ),
        )
